            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form method="POST" action="{{route('sponsors.create')}}" class="sponsors_create"  enctype="multipart/form-data">
                            {{csrf_field()}}
                                ...
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <form method="POST" action="{{route('sponsors.edit.new')}}" class="sponsors_edit"  enctype="multipart/form-data">
                                                {{csrf_field()}}
                                                {{method_field('PATCH')}}
                                                <div class="form-group">
...
@endif
                                ...
                                <tr id="row_sponsors{{$i->id}}">
                                    ...
                                    <td id="link_img_cont_{{$i->id}}">{{$i->link_image}}</td>
                                    <td>{{$i->created_at}}</td>
                                    <td>{{$i->updated_at}}</td>
                                    ...
                                    @if(Auth::user() && (Auth::user()->partition_id == 1 || Auth::user()->partition_id == 3))
                                        <td>
                                            <div class="m-btn-group m-btn-group--pill btn-group m-btn-group m-btn-group--pill btn-group-sm" role="group" aria-label="First group">
                                                <button onclick="getSponsors({{$i->id}})" type="button" data-toggle="modal" data-target="#slider_edit" class="m-btn btn btn-primary">
                                                    <i class="flaticon-edit-1"></i>
                                                </button>

                                                <button data-sponsor="{{$i->id}}" type="button" data-toggle="modal" data-target="#confirmModal" class="delete1 m-btn btn btn-danger">
...
//             $("#image_msg").html('').attr('style','display:none');
    ...
    function getSponsors($id) {
        $item_sponsor=$id;
        $.get('{{route('sponsors.edit.new')}}',{item_sponsor:$item_sponsor},function(data){
    ...
    $(".delete1").click(function(event){
        ...
        if (r == true) {
            $.get('{{route('sponsors.destroy.new')}}', {item_sponsor: $item_sponsor}, function (data) {
                if (data.status == true) {
